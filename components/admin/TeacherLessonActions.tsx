...
interface TeacherLessonActionsProps {
  lessonId: string;
...
export default function TeacherLessonActions({
  lessonId,
  currentStatus,
  currentLessonDate,
  currentScheduleTime,
}: TeacherLessonActionsProps) {
          ...
          body: JSON.stringify({
            ...
            schedule_time:
              resolution === "rescheduled"
                ? lessonTime
                : null,
            attendance_notes: notes.trim() || null,
          }),
        }
      );
      ...
      {open && action && (
        ...
        >
          ...
          {(action ===
            ...
            <div className="mt-5">
              <label
                htmlFor={`teacher-lesson-date-${lessonId}`}
                className="
                  font-sans
                  text-[11px]
                  font-medium
              ...
              <input
                id={`teacher-lesson-date-${lessonId}`}
                type="date"
                value={lessonDate}
                onChange={(event) =>
              ...
              <label
                htmlFor={`teacher-lesson-time-${lessonId}`}
                className="
                  mt-4
                  block
                  font-sans
              ...
              <input
                id={`teacher-lesson-time-${lessonId}`}
                type="time"
                value={lessonTime}
                onChange={(event) =>
          ...
          {action === "teacher_cancelled" && (
            <p className="mt-5 font-sans text-[12px] leading-5 text-[#777771]">
              The lesson will not be consumed. It can be
              rescheduled later if needed.
            </p>
          )}
          ...
          {currentStatus === "completed" &&
            action !== "completed" && (
              <p className="mt-5 font-sans text-[12px] leading-5 text-[#777771]">
                This lesson is currently recorded as completed.
                Saving another outcome will replace that status.
              </p>
            )}

          <div className="mt-5">
            <label
              htmlFor={`teacher-attendance-notes-${lessonId}`}
              className="
                font-sans
                text-[11px]
                font-medium
            ...
            <textarea
              id={`teacher-attendance-notes-${lessonId}`}
              value={notes}
              onChange={(event) =>
