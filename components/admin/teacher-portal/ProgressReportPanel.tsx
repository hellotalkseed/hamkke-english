...
export default function ProgressReportPanel({enrollmentStudentId,studentName,enrollmentNumber,enrollmentStatus,teacherName}:{enrollmentStudentId:string;studentName:string;packageName?:string;enrollmentNumber:string;enrollmentStatus:string;usedLessons?:number;totalLessons?:number;teacherName?:string}) {
  ...
  function openCurrent(){if(enrollmentStatus!=="completed")return;if(locked){setViewItem({enrollment_student_id:enrollmentStudentId,enrollment_number:enrollmentNumber,enrollment_status:enrollmentStatus,is_current:true,report});setModal("view")}else{setEditItem({enrollment_student_id:enrollmentStudentId,enrollment_number:enrollmentNumber,enrollment_status:enrollmentStatus,is_current:true,report});void loadObservations(enrollmentStudentId);setModal("edit")}}
        ...
        <table className="w-full border-collapse text-left">
          ...
          <tbody className="divide-y divide-[#EEE9E3]">
            <tr className="transition hover:bg-[#FCFBF9]">
              ...
              <td className="whitespace-nowrap px-5 py-4 text-right">{enrollmentStatus==="completed"?<button onClick={openCurrent} className="text-[12px] font-semibold text-[#5F7F63] hover:text-[#466149]">{locked?"View Report →":report.id?"Continue Report →":"Write Report →"}</button>:<span className="text-[12px] text-[#A19B94]">-</span>}</td>
            </tr>
