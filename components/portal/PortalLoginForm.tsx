...
export default function PortalLoginForm({ locale, denied }: { locale: Locale; denied: boolean }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // UX check only. The dashboard and RPC independently authorize access.
      const account = await supabase.from("portal_accounts").select("status")
        .eq("user_id", result.data.user.id).maybeSingle();
      if (account.error || account.data?.status !== "active") {
        await supabase.auth.signOut({ scope: "local" });
        setError(account.error ? t.connection : t.unavailable);
        return;
      }
  ...
  const inputClass = "mt-2 w-full rounded-xl border border-[#DCD8D2] bg-[#FFFDF8] px-4 py-3 outline-none focus:border-[#718A73] focus:ring-2 focus:ring-[#718A73]/20";
  return (
    <form onSubmit={submit} className="mt-9 space-y-6">
      <div><label htmlFor="portal-email" className="text-sm font-medium">{t.email}</label>
      ...
      <p className="text-sm leading-6 text-[#607568]">{t.help}</p>
    </form>
  );
}
