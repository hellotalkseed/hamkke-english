...
import PortalEnrollmentTable, { type EnrollmentDetail } from "@/components/portal/PortalEnrollmentTable";
...
const dashboardCopy = {
  ...
  zh: {
    ...
    myLessons: "我的课程", attendance: "出勤记录", reports: "学习报告", enrollment: "报名信息", settings: "设置", welcomeLine: "让我们继续一起成长，", scheduleSoon: "课程安排即将在这里开放。请通过平时使用的聊天软件向 Hamkke 确认下一节课。", attendanceSoon: "出勤记录即将在这里开放。当前套餐的剩余课时可在“我的课程”中查看。", reportsSoon: "目前还没有已完成的学习报告。完成后的学习报告会显示在这里。", adminSoon: "付款详情和合同即将在这里开放。", learnerName: "学员姓名", timezone: "学员时区", settingsNote: "这里的账号信息仅供查看。如需修改信息或获取密码帮助，请联系 Hamkke。",
...
export default async function PortalPage({ params, searchParams }: {
  ...
  if (view === "enrollment" && !failed && selectedId && packages.length > 0) {
    const admin = createAdminClient();
    const enrollmentIds = packages.map((item) => item.enrollment_id).filter((id): id is string => Boolean(id));
    const [{ data: payments }, { data: contracts }, { data: enrollmentLessons }] = await Promise.all([
      admin.from("payments").select("enrollment_id, amount, currency, payment_date, payment_method, status").in("enrollment_id", enrollmentIds),
      admin.from("contracts").select("enrollment_id, contract_number, status, agreement_date, accepted_by_name, accepted_by_relationship").in("enrollment_id", enrollmentIds),
      admin.from("lessons").select("id, enrollment_id, lesson_number, lesson_date, schedule_time, attendance_status, resolution").in("enrollment_id", enrollmentIds).order("lesson_date", { ascending: false }),
    ]);
    const paymentByEnrollment = new Map((payments ?? []).map((row) => [row.enrollment_id, row]));
    const contractByEnrollment = new Map((contracts ?? []).map((row) => [row.enrollment_id, row]));
    const lessonsByEnrollment = new Map<string, typeof enrollmentLessons>();
    for (const row of enrollmentLessons ?? []) {
      const list = lessonsByEnrollment.get(row.enrollment_id) ?? [];
      list.push(row);
      lessonsByEnrollment.set(row.enrollment_id, list);
    }
    const displayStatus = (value: string | null | undefined) => value ? value.replaceAll("_", " ").replace(/\b\w/g, (letter) => letter.toUpperCase()) : "-";
    const displayMethod = (value: string | null | undefined) => {
      if (!value) return "-";
      const normalized = value.trim().toLowerCase().replace(/[\s_-]+/g, "");
      return ({ gcash: "GCash", paypal: "PayPal", bank: "Bank transfer", banktransfer: "Bank transfer", sentbe: "SentBe" } as Record<string,string>)[normalized] ?? displayStatus(value);
    };
    const money = (amount: number | null | undefined, currency: string | null | undefined) => {
      if (amount === null || amount === undefined || !currency) return "-";
      try { return new Intl.NumberFormat(locale, { style: "currency", currency, maximumFractionDigits: currency === "KRW" ? 0 : 2 }).format(amount); }
      catch { return `${currency} ${number.format(amount)}`; }
    };
    enrollmentDetails = packages.map((item) => {
      const payment = paymentByEnrollment.get(item.enrollment_id!);
      const contract = contractByEnrollment.get(item.enrollment_id!);
      return {
        enrollmentId: item.enrollment_id!, enrollmentNumber: item.enrollment_number ?? "-", packageName: item.package_name ?? t.packages,
        enrollmentStatus: status(item), startDate: date(item.start_date), lessonDuration: item.lesson_duration === null ? t.notSet : t.minutes.replace("{count}", number.format(item.lesson_duration)),
        totalLessons: item.total_lessons, usedLessons: item.used_lessons, remainingLessons: item.remaining_lessons, isShared: item.is_shared,
        payment: payment ? { amount: money(payment.amount, payment.currency), status: displayStatus(payment.status), date: date(payment.payment_date), method: displayMethod(payment.payment_method) } : null,
        agreement: contract ? { number: contract.contract_number ?? "-", status: displayStatus(contract.status), date: date(contract.agreement_date), acceptedBy: contract.accepted_by_name ?? "-", relationship: displayStatus(contract.accepted_by_relationship), href: `/${locale}/portal/agreement?enrollment=${encodeURIComponent(item.enrollment_id!)}` } : null,
        attendance: (lessonsByEnrollment.get(item.enrollment_id!) ?? []).map((lesson) => ({
          lesson: lesson.lesson_number === null ? "-" : `${d.lessonLabel} ${number.format(lesson.lesson_number)}`,
          date: date(lesson.lesson_date), time: time(lesson.schedule_time), timezone: item.student_timezone?.replaceAll("_", " ") ?? "-",
          status: attendanceStatus(lesson.attendance_status ?? "scheduled", lesson.resolution ?? null),
        })),
      };
    });
  }
  ...
  <div className="mt-5 flex flex-col items-center sm:mt-6">
    ...
    <p lang="en" className="mt-2 text-center font-serif text-[34px] italic leading-[1.15] text-[#718A73] sm:text-[44px] xl:text-[48px]">
      From Small Talk to Big Ideas.
            ...
            {view === "enrollment" ? <section><PortalEnrollmentTable rows={enrollmentDetails} copy={{ enrollment: d.reference, package: d.package, status: d.statusLabel, lessons: d.myLessons, details: d.details, viewDetails: locale === "ko" ? "상세 보기" : locale === "zh" ? "查看详情" : locale === "ja" ? "詳細を見る" : "View Details", startDate: t.starts, duration: t.duration, lessonBalance: d.balance, payment: locale === "ko" ? "결제 정보" : locale === "zh" ? "付款信息" : locale === "ja" ? "お支払い情報" : "Payment Details", amount: locale === "ko" ? "결제 금액" : locale === "zh" ? "付款金额" : locale === "ja" ? "お支払い金額" : "Amount", paymentDate: locale === "ko" ? "결제일" : locale === "zh" ? "付款日期" : locale === "ja" ? "お支払い日" : "Payment Date", paymentMethod: locale === "ko" ? "결제 방법" : locale === "zh" ? "付款方式" : locale === "ja" ? "お支払い方法" : "Payment Method", agreement: locale === "ko" ? "수업 계약" : locale === "zh" ? "课程协议" : locale === "ja" ? "レッスン契約" : "Lesson Agreement", agreementNumber: locale === "ko" ? "계약 번호" : locale === "zh" ? "协议编号" : locale === "ja" ? "契約番号" : "Agreement Number", agreementDate: locale === "ko" ? "동의일" : locale === "zh" ? "同意日期" : locale === "ja" ? "同意日" : "Agreement Date", acceptedBy: locale === "ko" ? "동의자" : locale === "zh" ? "同意人" : locale === "ja" ? "同意者" : "Accepted By", attendance: d.attendanceHistory, lesson: d.lessonLabel, date: d.dateLabel, time: d.timeLabel, close: d.closeNotes, noAttendance: d.noAttendance, viewAgreement: locale === "ko" ? "수업 계약서 보기" : locale === "zh" ? "查看课程协议" : locale === "ja" ? "レッスン契約書を見る" : "View Agreement", viewAgreementHelp: locale === "ko" ? "동의한 전체 수업 계약서를 확인하세요." : locale === "zh" ? "查看您已同意的完整课程协议。" : locale === "ja" ? "同意済みのレッスン契約書全文を確認できます。" : "Read the full agreement you accepted.", contractLabel: locale === "ko" ? "계약서" : locale === "zh" ? "合同" : locale === "ja" ? "契約書" : "Contract", contractAction: locale === "ko" ? "보기" : locale === "zh" ? "查看" : locale === "ja" ? "表示" : "View" }} /></section> : activeTerms.length > 0 ? <section aria-label={d.current} className="space-y-5">
            ...
            {view === "enrollment" && <div className="pt-1">
              <Link href={`/${locale}/policy`} className={`inline-flex min-h-11 items-center gap-2 rounded-full border border-[#718A73]/40 px-5 py-2.5 text-sm font-medium text-[#31463A] transition hover:bg-[#EEF2EA] ${focus}`}>{d.fullPolicy}<ArrowUpRight size={16} aria-hidden="true" /></Link>
            </div>}
              </div>
            )}
